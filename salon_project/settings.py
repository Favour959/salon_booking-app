...
# Read from the environment in production; falls back to a dev-only key locally.
...
# In production, Render provides DATABASE_URL (Postgres). Locally it is unset,
# so we keep the SQLite default above.
...
# Compress + cache-bust static files, served by WhiteNoise.
STORAGES = {
    'default': {
...
# HTTPS origins allowed to submit forms (Render serves the app over HTTPS).
# Add your real domain here too if you attach one later.
...
DEFAULT_FROM_EMAIL = os.environ.get(
    ...
    "Fav_hie's GlowUp Salon <noreply@glowupsalon.local>",
)
